...
/**
 * Draw multiple line/area series on one canvas.
 * @param {string} canvasId - Canvas element ID
 * @param {Array<{data: number[], color: string, label?: string}>} series
 * @param {object} options - Optional settings
 * @param {number} options.maxValue - Max Y value (default: 100)
 * @param {boolean} options.showDot - Show current value dots (default: true)
 * @param {number} options.fillAlpha - Fill opacity 0-1 (default: 0.07)
 */
export function drawMultiChart(canvasId, series, options = {}) {
    const canvas = document.getElementById(canvasId);
    if (!canvas) return;

    const { maxValue = 100, showDot = true, fillAlpha = 0.07 } = options;
